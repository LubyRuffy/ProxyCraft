...
import { ArrowUpDown, Check, Loader2, TriangleAlert } from 'lucide-react';
  ...
  const columns = useMemo<ColumnDef<TrafficEntry>[]>(() => [
    {
      ...
      cell: ({ row }) => {
        ...
        const timedOut = entry.isTimeout;

        return (
          <div className="flex items-center justify-center">
            {timedOut ? (
              <TriangleAlert className="h-4 w-4 text-chart-5" aria-label="超时" role="img" />
            ) : isPending ? (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const entry = row.original;
        const tags = getTrafficTags(entry);
        return (
          <div className="flex flex-wrap gap-1">
            {tags.includes('https') ? <Badge variant="secondary">HTTPS</Badge> : null}
            {tags.includes('sse') ? <Badge variant="secondary">SSE</Badge> : null}
            {tags.includes('ai') ? <Badge variant="secondary">AI</Badge> : null}
          </div>
        );
      },
    },
